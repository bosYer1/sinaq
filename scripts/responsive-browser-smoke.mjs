  ...
  if (viewport.mobile) {
    ...
    const listView = await evaluate(client, `(() => {
      ...
      return {
        ...
        previewPresent: Boolean(document.querySelector('[data-map-preview="current-clubs"]')),
    ...
    assert(!listView.previewPresent, `${viewport.name}: static map preview leaked into mobile list view`, listView);
