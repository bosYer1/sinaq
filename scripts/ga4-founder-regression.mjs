...
assert.match(ga4, /analyticsdata\.googleapis\.com/, 'GA4 adapter must use the official Data API host');
assert.match(ga4, /runReport/, 'GA4 adapter must execute a report query');
assert.match(ga4, /normalizePrivateKey/, 'private-key normalization must remain explicit');
