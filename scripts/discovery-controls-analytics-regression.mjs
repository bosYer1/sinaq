...
assert.ok(explore.includes("if (isDesktop) cardRefs.current[id]?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });"), 'Marker selection may synchronize the desktop list but must not force-scroll the mobile list.');
assert.ok(!explore.includes('const needsMobileExpansion ='), 'Mobile marker selection must not auto-expand the club list.');
assert.ok(!explore.includes("block: 'center'"), 'Mobile marker selection must not pull the page away from the map.');
