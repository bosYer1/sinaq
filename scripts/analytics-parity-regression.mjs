...
const [card, link, detail, pageview, errorPage, notFound, posthog] = await Promise.all([
...
for (const event of ['phone_click', 'instagram_click', 'maps_click']) assert.ok(detail.includes(`eventType="${event}"`), `ClubDetail must keep ${event} conversion wiring`);
...
const combined = [card, link, detail, pageview, errorPage, notFound, posthog].join('\n');
