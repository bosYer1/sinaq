...
begin
  ...
  v_expected_prefix := 'https://uxcedpbumulpheglhlvs.supabase.co/storage/v1/object/public/club-images/owner-submissions/' || p_submission_id::text || '/';
  if exists (
    select 1
    from unnest(coalesce(v_images, '{}'::text[])) as image_url
    where image_url not like v_expected_prefix || '%'
       or image_url like '%?%'
       or image_url like '%#%'
  ) then
