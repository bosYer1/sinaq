...
function typeLabel(type: string) {
  return type === 'pc' ? 'PC' : 'PlayStation';
}

async function getPageData(slug: string, type: string) {
  if (type !== 'pc' && type !== 'playstation') return null;
  const [districts, clubs] = await Promise.all([
    getDistricts(),
    getClubs({ district: slug, type }),
  ]);
...
export async function generateMetadata({ params }: DistrictTypePageProps): Promise<Metadata> {
  ...
  const description = `${data.district.name} rayonundakı ${label} gaming klublarını GameYer-də müqayisə et. ${data.clubs.length} klubun ünvan, qiymət, iş saatı və xəritə məlumatlarına bax.`;
...
export default async function DistrictTypePage({ params }: DistrictTypePageProps) {
  ...
  const structuredData = {
    ...
    '@graph': [
      ...
      {
        '@type': 'ItemList',
        name: `${data.district.name} rayonunda ${label} klubları`,
        numberOfItems: data.clubs.length,
        itemListElement: data.clubs.map((club, index) => ({
          '@type': 'ListItem',
          position: index + 1,
          name: club.name,
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-10">
      ...
      <nav className="mb-5 text-xs text-muted" aria-label="Breadcrumb">
        ...
        <Link href={`/rayon/${data.district.slug}`} className="hover:text-ink">{data.district.name}</Link>{' '}
      ...
      <p className="mt-3 max-w-3xl text-sm leading-6 text-muted">
        {data.district.name} ərazisində {label} klub axtaranlar üçün {data.clubs.length} aktiv məkanı bir yerdə müqayisə et. Ünvan, xəritə, iş saatları və mövcud olduqda qiymət məlumatları klub səhifələrində göstərilir.
      </p>
      <div className="mt-7"><SeoClubList clubs={data.clubs} /></div>
      <div className="mt-8 flex flex-wrap gap-2">
        <Link href={`/?district=${encodeURIComponent(data.district.slug)}&type=${type}&view=map`} className="rounded-control bg-primary px-4 py-2 text-sm font-semibold text-white">Xəritədə göstər</Link>
        <Link href={`/rayon/${data.district.slug}`} className="rounded-control border border-border bg-surface px-4 py-2 text-sm font-semibold text-ink">{data.district.name} üzrə bütün klublar</Link>
