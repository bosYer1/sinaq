...
export default async function DistrictPage({ params }: DistrictPageProps) {
  ...
  return <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-10">
    ...
    <nav className="mb-5 text-xs text-muted" aria-label="Breadcrumb"><Link href="/" className="hover:text-ink">GameYer</Link> <span aria-hidden="true">/</span> <span>{district.name}</span></nav>
    <h1 className="font-display text-2xl font-bold text-ink sm:text-3xl">{district.name} rayonunda gaming klubları</h1>
    <p className="mt-3 max-w-3xl text-sm leading-6 text-muted">{district.name} rayonunda aktiv PC, kompüter və PlayStation klublarını müqayisə et. Hazırda {clubs.length} klub göstərilir; klub səhifələrində ünvan, xəritə, iş saatları və mövcud olduqda qiymətlər yer alır.</p>
    {(pcCount >= 2 || playStationCount >= 2) ? <div className="mt-5 flex flex-wrap gap-2">
      {pcCount >= 2 ? <Link href={`/rayon/${district.slug}/pc`} className="rounded-control border border-border bg-surface px-3 py-2 text-xs font-semibold text-ink">{district.name} PC klubları ({pcCount})</Link> : null}
      {playStationCount >= 2 ? <Link href={`/rayon/${district.slug}/playstation`} className="rounded-control border border-border bg-surface px-3 py-2 text-xs font-semibold text-ink">{district.name} PlayStation klubları ({playStationCount})</Link> : null}
    </div> : null}
    <div className="mt-7"><SeoClubList clubs={clubs} /></div>
    <div className="mt-8 flex flex-wrap gap-2"><Link href={`/?district=${encodeURIComponent(district.slug)}&view=map`} className="rounded-control bg-primary px-4 py-2 text-sm font-semibold text-white">Xəritədə göstər</Link><Link href="/rayon" className="rounded-control border border-border bg-surface px-4 py-2 text-sm font-semibold text-ink">Digər rayonlar</Link><Link href="/" className="rounded-control border border-border bg-surface px-4 py-2 text-sm font-semibold text-ink">Bütün klublar</Link></div>
  </div>;
}
