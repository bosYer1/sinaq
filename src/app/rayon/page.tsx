...
export default async function DistrictIndexPage() {
  ...
  return <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-10">
    ...
    <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{districts.map((district) => <Link key={district.id} href={`/rayon/${district.slug}`} className="rounded-card border border-border bg-surface p-4 shadow-card transition hover:border-border-strong hover:shadow-card-hover"><span className="font-display text-sm font-semibold text-ink">{district.name}</span><span className="mt-1 block text-xs text-muted">{district.count} aktiv gaming klubu →</span></Link>)}</div>
  </div>;
}
