...
export default async function AdminSubmissionsPage({ searchParams }: AdminSubmissionsPageProps) {
  ...
  let query = supabase
    ...
    .select('id,kind,club_id,club_name,message,contact_type,contact_value,status,created_at,reviewed_at,submitted_images')
  ...
  return (
    <div>
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <Link href="/admin/muracietler?status=pending" className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm text-gray-600 transition hover:border-[#7C5CFC]/40">Gözləyən: <span className="font-bold text-gray-900">{pendingCount}</span></Link>
      </div>
      ...
      <form method="get" className="mt-6 grid gap-3 rounded-xl border border-gray-200 bg-white p-4 md:grid-cols-[1fr_180px_180px_auto]">
        <div><label htmlFor="submission-q" className="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500">Klub adı</label><input id="submission-q" name="q" defaultValue={q} maxLength={120} placeholder="Klub adına görə axtar" className="h-10 w-full rounded-lg border border-gray-300 px-3 text-sm outline-none focus:border-[#7C5CFC]" /></div>
        <div><label htmlFor="submission-status" className="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500">Status</label><select id="submission-status" name="status" defaultValue={status ?? ''} className="h-10 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm"><option value="">Hamısı</option><option value="pending">Gözləyir</option><option value="reviewing">Yoxlanılır</option><option value="resolved">Həll olunub</option><option value="rejected">Rədd edilib</option></select></div>
        <div><label htmlFor="submission-kind" className="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500">Növ</label><select id="submission-kind" name="kind" defaultValue={kind ?? ''} className="h-10 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm"><option value="">Hamısı</option><option value="correction">Düzəliş</option><option value="new_club">Yeni klub</option><option value="owner_claim">Klub sahibi</option></select></div>
        <div className="flex items-end gap-2"><button type="submit" className="h-10 rounded-lg bg-[#7C5CFC] px-4 text-sm font-semibold text-white hover:bg-[#6A47F0]">Filtrlə</button>{hasFilters ? <Link href="/admin/muracietler" className="flex h-10 items-center rounded-lg border border-gray-300 bg-white px-4 text-sm font-medium text-gray-700 hover:bg-gray-50">Təmizlə</Link> : null}</div>
      </form>
            ...
            return (
              <article key={item.id} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-[#7C5CFC]/10 px-2.5 py-1 text-xs font-semibold text-[#6A47F0]">{KIND_LABELS[item.kind]}</span><span className="rounded-full bg-gray-100 px-2.5 py-1 text-xs font-semibold text-gray-600">{STATUS_LABELS[item.status]}</span>{(item.kind === 'owner_claim' || item.kind === 'correction') && !item.club_id ? <span className="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-800">Kluba bağlı deyil</span> : null}</div>
                ...
                {item.kind === 'owner_claim' ? <OwnerClaimApplyForm id={item.id} clubId={item.club_id} message={item.message} status={item.status} current={item.club_id ? currentValuesByClubId.get(item.club_id) : null} submittedImages={item.submitted_images ?? []} /> : null}
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3">
                  <form action={updateSubmissionStatus} className="flex flex-wrap items-center gap-2">
                    ...
                    <select id={`status-${item.id}`} name="status" defaultValue={item.status} className="h-9 rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-900"><option value="pending">Gözləyir</option><option value="reviewing">Yoxlanılır</option><option value="resolved">Həll olunub</option><option value="rejected">Rədd edilib</option></select>
                  ...
                  {completed ? <form action={deleteCompletedSubmission}><input type="hidden" name="id" value={item.id} /><button type="submit" className="h-9 rounded-lg border border-red-200 bg-white px-4 text-sm font-semibold text-red-600 transition hover:bg-red-50">Müraciəti sil</button></form> : null}
                </div>
              </article>
            );
          })}
