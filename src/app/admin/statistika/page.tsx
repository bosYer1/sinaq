...
type RecentVisit = { path: string; source: string; created_at: string; visitor: string; device: string; browser: string };
type AnalyticsData = {
  today_views: number;
    ...
    <div>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Real public trafik, sessiyalar, cihaz/brauzer bölgüsü, trafik mənbələri və CTA siqnalları. Admin hesabının öz aktivliyi sayılmır.</p>
        </div>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-xl border border-gray-200 bg-white p-6"><p className="text-sm text-gray-500">Bu gün baxış</p><p className="mt-2 text-4xl font-bold">{stats.today_views}</p><p className="mt-2 text-xs text-gray-500">{stats.today_visitors} unikal sessiya</p></div>
        <div className="rounded-xl border border-gray-200 bg-white p-6"><p className="text-sm text-gray-500">Son 7 gün</p><div className="mt-2 flex items-end justify-between gap-3"><p className="text-4xl font-bold">{stats.views_7d}</p><span className="text-xs font-bold text-gray-500">{growth(stats.views_7d, stats.prev_views_7d)}</span></div><p className="mt-2 text-xs text-gray-500">{stats.visitors_7d} unikal sessiya · əvvəlki 7 gün: {stats.prev_views_7d}</p></div>
        <div className="rounded-xl border border-gray-200 bg-white p-6"><p className="text-sm text-gray-500">Son 30 gün</p><div className="mt-2 flex items-end justify-between gap-3"><p className="text-4xl font-bold">{stats.views_30d}</p><span className="text-xs font-bold text-gray-500">{growth(stats.views_30d, stats.prev_views_30d)}</span></div><p className="mt-2 text-xs text-gray-500">{stats.visitors_30d} unikal sessiya · əvvəlki 30 gün: {stats.prev_views_30d}</p></div>
        <div className="rounded-xl border border-gray-200 bg-white p-6"><p className="text-sm text-gray-500">Təkrar sessiyalar</p><p className="mt-2 text-4xl font-bold">{stats.returning_visitors_30d}</p><p className="mt-2 text-xs text-gray-500">Yeni: {stats.new_visitors_30d} · son 30 gün</p></div>
      </div>

      <section className="mt-6 rounded-xl border border-gray-200 bg-white p-6">
        <h2 className="text-lg font-bold">Canlı aktivlik</h2><p className="mt-1 text-sm text-gray-500">Son public baxışlara əsaslanan təxmini aktiv sessiya sayı.</p>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-2">
        {[['Cihazlar', stats.devices], ['Brauzerlər', stats.browsers]].map(([title, items]) => (
          <section key={title as string} className="rounded-xl border border-gray-200 bg-white p-6">
            ...
            <div className="mt-4 divide-y divide-gray-100">{(items as Breakdown[]).length === 0 ? <p className="py-3 text-sm text-gray-500">Hələ məlumat yoxdur.</p> : (items as Breakdown[]).map((item) => <div key={item.name} className="flex items-center justify-between py-3"><div><p className="text-sm font-semibold">{item.name}</p><p className="text-xs text-gray-500">{item.visitors} sessiya</p></div><span className="text-sm font-bold">{item.views} baxış</span></div>)}</div>
          </section>
        ))}
      </div>

      <section className="mt-6 overflow-hidden rounded-xl border border-gray-200 bg-white">
        <div className="border-b border-gray-100 px-6 py-5"><h2 className="text-lg font-bold">Son baxışlar</h2><p className="mt-1 text-sm text-gray-500">Son 50 public baxış. Sessiya identifikatoru qısaldılmış formada göstərilir.</p></div>
        <div className="overflow-x-auto"><table className="min-w-full divide-y divide-gray-100 text-left text-sm"><thead className="bg-gray-50 text-xs font-semibold uppercase tracking-wide text-gray-500"><tr><th className="px-5 py-3">Vaxt</th><th className="px-5 py-3">Sessiya</th><th className="px-5 py-3">Cihaz</th><th className="px-5 py-3">Səhifə</th><th className="px-5 py-3">Mənbə</th></tr></thead><tbody className="divide-y divide-gray-100">{stats.recent_visits.length === 0 ? <tr><td colSpan={5} className="px-5 py-6 text-gray-500">Hələ yeni baxış qeydi yoxdur.</td></tr> : stats.recent_visits.map((visit, index) => <tr key={`${visit.created_at}-${visit.visitor}-${index}`} className="align-top"><td className="whitespace-nowrap px-5 py-3 text-xs text-gray-500">{formatVisitTime(visit.created_at)}</td><td className="whitespace-nowrap px-5 py-3 font-mono text-xs text-gray-500">{visit.visitor}</td><td className="whitespace-nowrap px-5 py-3 text-xs">{visit.device} · {visit.browser}</td><td className="max-w-[360px] truncate px-5 py-3 font-medium" title={visit.path}>{visit.path}</td><td className="whitespace-nowrap px-5 py-3 text-xs text-gray-600">{sourceLabel(visit.source)}</td></tr>)}</tbody></table></div>
      </section>

      <section className="mt-6 rounded-xl border border-gray-200 bg-white p-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-lg font-bold">Klub hərəkətləri</h2><p className="mt-1 text-sm text-gray-500">Son 30 gündə real niyyət siqnalları.</p></div><span className="text-sm font-bold">Cəmi: {totalActions}</span></div><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-lg bg-gray-50 p-4"><p className="text-xs text-gray-500">Maps</p><p className="mt-2 text-3xl font-bold">{stats.cta_30d.maps_click}</p></div><div className="rounded-lg bg-gray-50 p-4"><p className="text-xs text-gray-500">Telefon</p><p className="mt-2 text-3xl font-bold">{stats.cta_30d.phone_click}</p></div><div className="rounded-lg bg-gray-50 p-4"><p className="text-xs text-gray-500">Instagram</p><p className="mt-2 text-3xl font-bold">{stats.cta_30d.instagram_click}</p></div></div><div className="mt-5 divide-y divide-gray-100">{stats.top_action_clubs.length === 0 ? <p className="py-3 text-sm text-gray-500">Hələ klub hərəkəti yoxdur.</p> : stats.top_action_clubs.map((item, index) => <div key={item.club_slug} className="flex items-center gap-3 py-2.5"><span className="w-5 text-xs font-bold text-gray-400">{index + 1}</span><Link href={`/klub/${item.club_slug}`} target="_blank" className="min-w-0 flex-1 truncate text-sm font-semibold text-[#6A47F0] hover:underline">{item.club_slug}</Link><span className="text-sm font-bold">{item.actions}</span></div>)}</div></section>

      <div className="mt-8 grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <section className="rounded-xl border border-gray-200 bg-white p-6"><h2 className="text-lg font-bold">Son 14 gün</h2><p className="mt-1 text-sm text-gray-500">Günlük baxış / unikal sessiya.</p><div className="mt-6 space-y-3">{stats.daily.length === 0 ? <p className="text-sm text-gray-500">Hələ statistika yoxdur.</p> : stats.daily.map((item) => <div key={item.date} className="grid grid-cols-[84px_1fr_78px] items-center gap-3 text-sm"><span className="text-xs text-gray-500">{new Intl.DateTimeFormat('az-AZ', { day: '2-digit', month: 'short' }).format(new Date(`${item.date}T12:00:00+04:00`))}</span><div className="h-3 overflow-hidden rounded-full bg-gray-100"><div className="h-full rounded-full bg-[#7C5CFC]" style={{ width: `${Math.max(3, Math.round((item.views / maxDailyViews) * 100))}%` }} /></div><span className="text-right text-xs font-semibold">{item.views} / {item.visitors}</span></div>)}</div></section>
        <section className="rounded-xl border border-gray-200 bg-white p-6"><h2 className="text-lg font-bold">Ən çox baxılan səhifələr</h2><p className="mt-1 text-sm text-gray-500">Son 30 gün.</p><div className="mt-5 divide-y divide-gray-100">{stats.top_pages.length === 0 ? <p className="py-3 text-sm text-gray-500">Hələ statistika yoxdur.</p> : stats.top_pages.map((item, index) => <div key={item.path} className="flex items-center gap-3 py-3"><span className="w-6 text-xs font-bold text-gray-400">{index + 1}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{item.path}</p><p className="mt-0.5 text-xs text-gray-500">{item.visitors} sessiya</p></div><span className="text-sm font-bold">{item.views}</span></div>)}</div></section>
      </div>

      <section className="mt-6 rounded-xl border border-gray-200 bg-white p-6"><h2 className="text-lg font-bold">Trafik mənbələri</h2><p className="mt-1 text-sm text-gray-500">Son 30 gündə baxışların haradan gəldiyi.</p><div className="mt-5 space-y-3">{stats.top_sources.length === 0 ? <p className="py-3 text-sm text-gray-500">Hələ mənbə məlumatı yoxdur.</p> : stats.top_sources.map((item) => <div key={item.source} className="grid gap-2 sm:grid-cols-[160px_1fr_110px] sm:items-center"><div className="min-w-0"><p className="truncate text-sm font-semibold">{sourceLabel(item.source)}</p>{item.source !== 'direct' ? <p className="truncate text-[11px] text-gray-400">{item.source}</p> : null}</div><div className="h-2.5 overflow-hidden rounded-full bg-gray-100"><div className="h-full rounded-full bg-[#7C5CFC]" style={{ width: `${Math.max(3, Math.round((item.views / maxSourceViews) * 100))}%` }} /></div><p className="text-xs text-gray-500 sm:text-right"><strong className="text-gray-900">{item.views}</strong> baxış · {item.visitors} sessiya</p></div>)}</div></section>

      <div className="mt-6 rounded-xl border border-gray-200 bg-white p-5 text-sm text-gray-600"><strong className="text-gray-900">Necə hesablanır?</strong> Public baxışlarda anonim browser sessiya identifikatoru, baxılan səhifə, trafik mənbəyi və ümumi cihaz/brauzer məlumatı saxlanır. Admin hesabı ilə daxil olduğun halda public səhifə baxışları və CTA klikləri analytics-ə yazılmır. “Unikal” göstəricilər anonim sessiya ID-si əsasında hesablanır və real şəxs sayı kimi təqdim edilmir.</div>
    </div>
  );
}
