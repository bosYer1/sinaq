...
export default async function CheapPcClubsPage() {
  ...
  const structuredData = {
    ...
    '@graph': [
      ...
      { '@type': 'ItemList', name: 'Bakıda 2 AZN-dək PC klubları', numberOfItems: clubs.length, itemListElement: clubs.map((club, index) => ({ '@type': 'ListItem', position: index + 1, name: club.name, url: `${siteUrl}/klub/${club.slug}` })) },
    ],
  };
