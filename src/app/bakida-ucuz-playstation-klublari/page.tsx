...
export async function generateMetadata(): Promise<Metadata> {
  const clubs = await getCheapPlayStationClubs();
  const title = 'Bakıda ucuz PlayStation klubları — 3 AZN-dək qiymətlər';
  const description = 'Bakıda ucuz PlayStation klub axtarırsan? Saatlıq qiyməti 3 AZN-dək olan PS4 və PS5 gaming klublarını ünvan, rayon, iş saatı və xəritə ilə müqayisə et.';
  return {
    title,
    description,
...
export default async function CheapPlayStationClubsPage() {
  const clubs = [...(await getCheapPlayStationClubs())].sort((a, b) => psMinPrice(a) - psMinPrice(b));
  ...
  const structuredData = {
    ...
    '@graph': [
      ...
      ...(clubs.length > 0 ? [{ '@type': 'ItemList', name: 'Bakıda 3 AZN-dək PlayStation klubları', numberOfItems: clubs.length, itemListElement: clubs.map((club, index) => ({ '@type': 'ListItem', position: index + 1, name: club.name, url: `${siteUrl}/klub/${club.slug}` })) }] : []),
    ],
  };
  ...
  return <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-10">
    ...
    {clubs.length > 0 ? <div className="mt-7"><SeoClubList clubs={clubs} /></div> : <div className="mt-7 rounded-card border border-border bg-surface p-5 text-sm text-muted">Hazırda 3 AZN-dək təsdiqlənmiş PlayStation tarifi olan klub yoxdur. Digər PlayStation klublarına və ümumi qiymət səhifəsinə bax.</div>}
