...
async function fetchPostHogMetrics(range: DateRange): Promise<PostHogMetrics> {
  ...
  try {
    const [overviewRows, campaignRows, clubRows, trendRows, healthRows, retentionRows, funnelRows, cohortRows] = await Promise.all([
      ...
      queryHogQL(host, projectId, apiKey, `SELECT maxIf(timestamp, ${publicScope}) AS latest_event_at, countIf(${publicScope}) AS public_events, countIf(properties.gameyer_traffic_scope = 'test') AS test_events, countIf(${publicScope} AND empty(properties.gameyer_session_landing_path)) AS missing_session, countIf(${publicScope} AND notEmpty(properties.gameyer_first_fbclid) AND empty(properties.gameyer_first_utm_campaign)) AS missing_campaign, countIf(${publicScope} AND event = 'search_query' AND properties.no_results = true) AS no_result_searches, countIf(${publicScope} AND properties.$virt_is_bot = true) AS bot_events, uniqIf(properties.$session_id, ${publicScope} AND empty(properties.gameyer_first_referrer) AND empty(properties.gameyer_first_utm_source)) AS source_missing_sessions FROM events WHERE timestamp >= toDateTime('${from}') AND timestamp < toDateTime('${to}')`),
      queryHogQL(host, projectId, apiKey, `SELECT count() AS users, countIf(person_sessions >= 2) AS returning_users, countIf(person_sessions >= 3) AS three_session_users, round(avg(person_sessions), 2) AS sessions_per_user FROM (SELECT person_id, uniq(properties.$session_id) AS person_sessions FROM events WHERE timestamp >= toDateTime('${from}') AND timestamp < toDateTime('${to}') AND ${publicScope} AND event = '$pageview' GROUP BY person_id)`),
    ...
    return {
      ...
      tracking: { latestEventAt: typeof health.latest_event_at === 'string' ? health.latest_event_at : null, publicEvents: numberValue(health.public_events), testEvents: numberValue(health.test_events), missingSessionAttribution: numberValue(health.missing_session), missingCampaignAttribution: numberValue(health.missing_campaign), noResultSearches: numberValue(health.no_result_searches), botEvents: numberValue(health.bot_events), sourceMissingSessions: numberValue(health.source_missing_sessions), attributionCompleteness: rate(numberValue(health.public_events) - numberValue(health.missing_session), numberValue(health.public_events)) },
