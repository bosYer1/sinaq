...
async function runReport(
  ...
  const url = `${GA4_API_ROOT}/properties/${propertyId}:runReport`;
...
async function loadGa4Metrics(range: DateRange): Promise<Ga4Metrics> {
  ...
  const missing = [
    ...
    ['GOOGLE_ANALYTICS_CLIENT_EMAIL', clientEmail],
    ['GOOGLE_ANALYTICS_PRIVATE_KEY', privateKey],
  ].filter(([, value]) => !value).map(([name]) => name);

  const unavailable = (detail: string): Ga4Metrics => ({
  ...
  if (missing.length > 0) return unavailable(`Konfiqurasiya yoxdur: ${missing.join(', ')}`);
  ...
  try {
    ...
    return {
      status: providerStatus('ga4', 'ready', 'GA4 Data API · server-side read-only'),
      sessions: metric(current.sessions, previous.sessions),
      activeUsers: metric(current.activeUsers, previous.activeUsers),
      totalUsers: metric(current.totalUsers, previous.totalUsers),
      ...
      eventCount: metric(current.eventCount, previous.eventCount),
      keyEvents: metric(current.keyEvents, previous.keyEvents),
    };
  } catch (error) {
...
const cachedGa4Metrics = unstable_cache(
  ...
  ['founder-analytics-ga4-v1'],
  { revalidate: 300 },
);

export async function getGa4Metrics(range: DateRange) {
  return cachedGa4Metrics(JSON.stringify(range));
}
