...
test('acquisition groups paid and organic campaign rows without losing totals', () => {
  ...
  assert.equal(acquisitionChannel('google', 'organic'), 'Organic');
...
test('CEO signals expose provider unavailability instead of fake metrics', () => {
  ...
  const supabase: SupabaseMetrics = {
    ...
    qualityBacklog: [],
    firstPartyIntent: { available: true, detail: 'test', events: 0, browserVisitors: 0, phoneClicks: 0, instagramClicks: 0, mapsClicks: 0, whatsappBookingClicks: 0 },
  };
