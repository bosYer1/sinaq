    ...
    <form action={action} className="space-y-5">
        ...
        <div className="mt-4 space-y-3">
          {types.map((type) => {
            ...
            return (
              <div
                key={type.id}
                className="grid gap-3 rounded-lg border border-gray-200 p-4 md:grid-cols-[150px_1fr_1fr_120px]"
              >
                <label className="flex items-center gap-2 text-sm font-semibold">
                  <input
                    type="checkbox"
                    name={`type_enabled_${type.id}`}
                    defaultChecked={Boolean(pricing)}
                  />
                ...
                <label className="text-xs font-medium text-gray-600">
                  ...
                  <input
                    name={`price_from_${type.id}`}
                    type="number"
                    step="0.01"
                    min="0"
                    defaultValue={pricing?.price_from ?? ''}
                    className={inputClass}
                  />
                </label>
                ...
                <label className="text-xs font-medium text-gray-600">
                  ...
                  <input
                    name={`price_to_${type.id}`}
                    type="number"
                    step="0.01"
                    min="0"
                    defaultValue={pricing?.price_to ?? ''}
                    className={inputClass}
                  />
                </label>
                ...
                <label className="text-xs font-medium text-gray-600">
                  ...
                  <input
                    name={`unit_${type.id}`}
                    defaultValue={pricing?.unit ?? 'saat'}
                    className={inputClass}
                  />
                </label>
              </div>
        ...
        <div className="mt-4 space-y-2">
          {DAYS.map((label, day) => {
            ...
            return (
              <div
                key={label}
                className="grid items-center gap-3 rounded-lg border border-gray-200 p-3 sm:grid-cols-[170px_110px_1fr_1fr]"
              >
                ...
                <label className="flex items-center gap-2 text-xs text-gray-600">
                  <input
                    type="checkbox"
                    name={`day_closed_${day}`}
                    defaultChecked={hours?.is_closed ?? false}
                  />
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-5">
        ...
        <p className="mt-1 text-xs text-gray-500">
          Hər sətrə bir şəkil URL-i yaz.
          Birinci URL cover şəkli olacaq.
        </p>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <label className="flex items-center gap-2 text-sm font-medium">
            <input
              type="checkbox"
              name="is_active"
              defaultChecked={club?.is_active ?? true}
            />
          ...
          <label className="flex items-center gap-2 text-sm font-medium">
            <input
              type="checkbox"
              name="is_premium"
              defaultChecked={club?.is_premium ?? false}
            />
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="text-sm font-medium">
              ...
              <input
                ...
                type="number"
                min="0"
                defaultValue={club?.rating_count ?? 0}
                className={inputClass}
              />
            </label>
          </div>
      ...
      <div className="sticky bottom-4 flex justify-end rounded-xl border border-gray-200 bg-white/95 p-3 shadow-lg backdrop-blur">
        <button
          type="submit"
          className="rounded-lg bg-[#7C5CFC] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#6A47F0]"
        >
          {submitLabel}
        </button>
      </div>
