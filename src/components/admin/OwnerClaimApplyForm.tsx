...
export function OwnerClaimApplyForm({ id, clubId, message, status }: OwnerClaimApplyFormProps) {
    ...
    <div className="mt-4 space-y-3">
      {hasApplicableField ? (
        <form action={applyOwnerClaimFields} className="rounded-lg border border-[#7C5CFC]/20 bg-[#7C5CFC]/5 p-3">
          ...
          <p className="mt-1 text-xs leading-5 text-gray-600">Yalnız rəsmi kanaldan ayrıca təsdiqlədiyin sahələri seç. Seçilməyən məlumat klub profilinə toxunmur.</p>
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {instagram ? (
              <label className="flex items-start gap-2 rounded-lg border border-gray-200 bg-white p-3 text-sm text-gray-800">
                ...
                <span><strong>Instagram</strong><span className="mt-0.5 block break-all text-xs font-normal text-gray-500">{instagram}</span></span>
              </label>
            ) : null}
            {pcPrice != null ? (
              <label className="flex items-start gap-2 rounded-lg border border-gray-200 bg-white p-3 text-sm text-gray-800">
                <input type="checkbox" name="apply_pc_price" className="mt-0.5" />
                <span><strong>PC qiyməti</strong><span className="mt-0.5 block text-xs font-normal text-gray-500">{pcPrice} AZN/saat</span></span>
              </label>
            ) : null}
            {psPrice != null ? (
              <label className="flex items-start gap-2 rounded-lg border border-gray-200 bg-white p-3 text-sm text-gray-800">
                <input type="checkbox" name="apply_ps_price" className="mt-0.5" />
                <span><strong>PlayStation qiyməti</strong><span className="mt-0.5 block text-xs font-normal text-gray-500">{psPrice} AZN/saat</span></span>
              </label>
            ) : null}
            {hours ? (
              <label className="flex items-start gap-2 rounded-lg border border-gray-200 bg-white p-3 text-sm text-gray-800">
                <input type="checkbox" name="apply_hours" className="mt-0.5" />
                <span><strong>İş saatları</strong><span className="mt-0.5 block text-xs font-normal text-gray-500">{claim.hours}</span></span>
              </label>
            ) : claim.hours ? (
