'use client';

import type { District } from '@/types/database';
import { useFilters } from '@/hooks/useFilters';
import { cn } from '@/lib/utils';

export function DistrictFilter({ districts }: { districts: District[] }) {
  const { filters, setDistrict } = useFilters();

  return (
    <div className="relative shrink-0">
      <select
        value={filters.district ?? ''}
        onChange={(e) => setDistrict(e.target.value || undefined)}
        className={cn(
          'h-10 min-w-[108px] appearance-none rounded-xl border border-border bg-surface pl-3 pr-8 text-sm text-ink transition md:h-9',
          'hover:border-border-strong focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/10',
          filters.district && 'border-primary/45 bg-primary/10 text-[#D7CAFF]',
        )}
        aria-label="Rayona görə filtr"
      >
        <option value="">Rayonlar</option>
        {districts.map((d) => (
          <option key={d.id} value={d.slug}>{d.name}</option>
        ))}
      </select>
      <svg viewBox="0 0 20 20" fill="none" className="pointer-events-none absolute right-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted">
        <path d="M5 7.5l5 5 5-5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </div>
  );
}
