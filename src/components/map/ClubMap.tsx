...
// Use OpenStreetMap's standard tiles so place/street labels follow the local OSM
// names instead of a third-party renderer's language fallback.
...
function createClubIcon(club: ClubWithDistance, isActive: boolean) {
  ...
  return L.divIcon({ className: 'gameyer-marker', html: svg, iconSize: [size, size], iconAnchor: [size / 2, size], popupAnchor: [0, -size] });
}
...
function createPopupContent(club: ClubWithDistance) {
  ...
  const typeSlugs = inferClubTypeSlugs(club);
  if (typeSlugs.length > 0) { const types = document.createElement('div'); types.className = 'mt-2 flex flex-wrap gap-1 text-[11px] text-muted'; for (const slug of typeSlugs) appendText(types, 'span', 'rounded bg-surface-alt px-1.5 py-0.5', slug === 'pc' ? 'PC' : 'PlayStation'); root.appendChild(types); }
  const hasHours = club.opening_hours.length > 0; const openNow = hasHours ? isClubOpenNow(club.opening_hours) : false; const statusLabel = !hasHours ? 'İş saatı məlum deyil' : openNow ? 'Açıqdır' : 'Bağlıdır';
  const cheapest = [...club.pricing].filter((pricing) => pricing.price_from > 0).sort((a, b) => a.price_from - b.price_from)[0];
  const meta = document.createElement('div'); meta.className = 'mt-2 flex items-center justify-between gap-3';
