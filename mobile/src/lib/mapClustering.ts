...
export type ClubMapPoint =
  | { kind: 'club'; id: string; club: MappableClub; latitude: number; longitude: number }
  | { kind: 'cluster'; id: string; clubs: MappableClub[]; latitude: number; longitude: number };

const LATITUDE_CELLS = 8;
const LONGITUDE_CELLS = 6;
const MINIMUM_CELL_SIZE = 0.00012;

export function clusterClubs(clubs: MappableClub[], region: Region, selectedClubId: string | null): ClubMapPoint[] {
  ...
  const groups = new Map<string, MappableClub[]>();
  const selected = clubs.find((club) => club.id === selectedClubId);

  for (const club of clubs) {
    if (club.id === selectedClubId) continue;
